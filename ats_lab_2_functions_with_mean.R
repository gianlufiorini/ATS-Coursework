...
KF <-function(y, omega, phi, sigma_e, sigma_eta){
...
loglikelihood <- function(par,y){
  ...
  obj = KF(y, par[1],par[2],par[3], par[4])$llk
...
estimator <- function(y,par){
  ...
  hat_sigma_eta <- hat$par[4]
  
  theta_hat <- c(hat_omega, hat_phi,hat_sigma_e,hat_sigma_eta)
  
  
  ###Create a list with ALL the optimized parameters
  theta_list <- list(hat_omega = hat_omega,
                     hat_phi = hat_phi,
