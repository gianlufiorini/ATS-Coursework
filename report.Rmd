...
```{r, fig.cap="ACF for standardized innovations for $y_$2"}
...
On the same note, it is interesting to note that the $\alpha$ parameter in this model is very similar to the steady-state Kalman gain for our Kalman Filter, which is equal to 0.5011753.
...
\footnotetext{Note that in GAS models, since the estimated state is a deterministic function of the past, the subscript $\|t-1$ used for the Kalman filter can be (and is) dropped, i.e. for GAS models $\mu_{t|t-1}=\mu_t$}.
...
ggplot(gf), aes(x = time, y = value, color = name, linewidth = name)) +
  geom_line() +
  theme_minimal() +
  theme(legend.text = element_text(size = 10)) +
  labs(
