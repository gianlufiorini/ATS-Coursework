...
\newpage
```{r}
ggplot(df_st_v[df_st_v$name == "v1",], aes(x = time, y = v)) +
  geom_line() +
...
```
```{r, fig.height=4, fig.width=4, fig.align='center', results='hide', fig.pos="H"}
par(cex.lab = .7,
    cex.axis = .7)
qqPlot(st_v1,
...
```
```{r}
ggplot(df_st_v[df_st_v$name == "v2",], aes(x = time, y = v)) +
  geom_line() +
  geom_hline(yintercept = c(-1.96,1.96),
             linetype = "dashed",
             col = "blue") +
